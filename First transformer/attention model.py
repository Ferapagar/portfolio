...
# generate the characters list
chars = sorted(list(set(text)))
vocab_size = len(chars)

# mappings between chars and indices
stoi = {ch:i for i,ch in enumerate(chars)} 
itos = {i:ch for i,ch in enumerate(chars)} 
# encoder and decoder: iterative mappings
encode = lambda s: [stoi[c] for c in s]
decode = lambda l: ''.join([itos[i] for i in l])



#encode to pytorch tensor and split into train/validation
...
for iter in range(max_iters): 
    ...
    if iter % eval_interval == 0:
        ...
        now = time.time()
        print(f"Step {iter} (time: {now-then:.2f}): train loss {losses['train']:.4f}, val losses {losses['val']:.4f}")
        then = now
